...
bool moveSoldier(shared_ptr<soldier> pSoldier, World world)
{
    ...
    if(chooseMove%4==0)//going up
    {
        if(i!=1)
        {
            ...
            if(world.items.find(temp)->second.name!='s'&&world.items.find(temp)->second.name!='p'&&world.items.find(temp)->second.name!='r'&&world.items.find(temp)->second.name!='M')
            {
    ...
    if(chooseMove%4==3)
    {
        if(j!=1)
        {
            Position temp;
            temp.pos=make_tuple(i,j-1);
            if(world.items.find(temp)->second.name!='s'&&world.items.find(temp)->second.name!='p'&&world.items.find(temp)->second.name!='r'&&world.items.find(temp)->second.name!='M')// check if it is not a mountain or an ally
            {
...
Action actionPlayerOne(const World world) { // this player plays with a basic strategy
    ...
    if(get<0>(k.pos)!=1)
    {
        ...
        else if(world.items.find(k)->second.name=='s') // this means that we need to change the leader to be able to reach the flag so we will sacrifice the current leader to be able to send another in the same path
        {
...
int validate(Action action)
{
    ...
    if(itemsMap[action.to].name<91&&itemsMap[action.from].name<91&&itemsMap[action.to].name>64&&itemsMap[action.from].name>64)
    {
        return -1;
    }
    if(itemsMap[action.to].name>91&&itemsMap[action.from].name>91&&itemsMap[action.to].name<173&&itemsMap[action.from].name<173)
    {
        return -1;
    }
...
World makeItDarkForP0(World world){
    ...
    for(int i=0;i<=15;i++)
    {
        for(int j=0;j<=15;j++)
        {
            Position k;
            k.pos=make_tuple(i,j);
            if(world.items.find(k)->second.name!='s'&&world.items.find(k)->second.name!='p'&&world.items.find(k)->second.name!='r'&&checkForUnits0(k))
            {
                units u;
                u.position=k;
                u.name='D';
...
World makeItDarkForP1(World world){
    ...
    for(int i=0;i<=15;i++)
    {
        for(int j=0;j<=15;j++)
        {
            Position k;
            k.pos=make_tuple(i,j);
            if(world.items.find(k)->second.name!='S'&&world.items.find(k)->second.name!='P'&&world.items.find(k)->second.name!='R'&&checkForUnits1(k))
            {
                units u;
                u.position=k;
                u.name='D';
...
int main() {
    ...
    while (!endGame) {
        ...
        auto[action0, timeout0] = waitPlayer(actionPlayerZero, makeItDarkForP0(world));//Item 3.a.3: requesting the action from each player
